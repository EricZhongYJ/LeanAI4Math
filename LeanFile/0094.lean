import Mathlib
/-! Question:
2.5.16 . 设 $k$ 是域, $f\left(x_{1}, x_{2}\right)$ 和 $g\left(x_{1}, x_{2}\right)$ 是 $k\left[x_{1}, x_{2}\right]$ 中两个互素的多项式. 求证在 $k$ 的任意扩域中 $f\left(x_{1}, x_{2}\right)=0$ 和 $g\left(x_{1}, x_{2}\right)=0$ 均只有有限多公共解 $\left(x_{1}, x_{2}\right)$.
-/
