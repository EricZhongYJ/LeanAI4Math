import Mathlib
/-! Question:
6. Let $F \leq K \leq E$, with $S$ a transcendence basis for $K / F$ and $T$ a transcendence basis for $E / K$. Show that $S \cup T$ is a transcendence basis for $E / F$. Thus if $\operatorname{tr}$ deg abbreviates transcendence degree,then by Problem 5, $\operatorname{tr} \operatorname{deg}(E / F)=\operatorname{tr} \operatorname{deg}(K / F)+\operatorname{tr} \operatorname{deg}(E / K) \text {. }$
-/
...
/-! Informal proof:
1.  Verify the definition, i.e. algebraic independent and E is algebraic over F
2.  To show the union is algebraic independent, suppose we have a polynomial $p$ such that $p(s, t) = 0$, hence $p(s, x)$ is a polynomial in which $s$ eval to $0$. By algebraic independence of $s$, we have $p(s, x) = 0$ for all $x$. Notice that every coeffient of $p(s, x)$ is a evaluation on $s$ of a polynomial in E, hence $p(s, x) = 0$ implies every coeffient is zero, hence $p = 0$.
3. To show $E$ is algebraic over $F(s, t)$, just use the transitivity of algebraic extension and $K$ is algebraic over $F(s)$, hence $K(t)$ is algebraic over $F(t, s)$, hence $E$ is algebraic over $F(t, s)$.
