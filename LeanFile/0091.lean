import Mathlib
/-! Question:
*23. Call an element in $S_{n}$ a transposition if it interchanges two elements, leaving the others fixed. Show that any element in $S_{n}$ is a product of transpositions.
-/
...
/-! Informal proof:
1. let $\sigma \in S_n$. If all points are fixed by $\sigma$, then it is $id$, just use an empty product.
2. Otherwise, random choose an element $a$ which is not fixed by $\sigma$.  Recursively, write $\sigma' := (a, \sigma) \sigma$ as a product of transpositions and add a $(a, \sigma a)$ to the head of product produce the product we need. It's easy to prove that $\sigma'$ has strictly more fixed points than $\sigma$, so the recursion is terminated.
-/
