import Mathlib
/-! Question:
1. Show that a partially ordered set $S$ is a complete lattice if and only if it has a greatest element and every nonempty subset of $S$ has a greatest lower bound.
-/
