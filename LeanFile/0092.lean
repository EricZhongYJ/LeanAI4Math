...
/-! Informal proof:
- If $cl$ != $[sr]$, we will show $\exists i, cl = [r^i]$. Choose an element in $cl$, if $ele = r^i$, then $cl = [r^i]$. Actually, if $i \le k$ then we just choose $i$, otherwise we choose $2 k + 1 - i$, it's easy to verify $[r^i] = [r^{2 k + 1 - i}]$.
- Otherwise, we only need to show every $s r^i$ are conjugated. Take $r^j$ and it' s easy to show we only need $j + j = i \space mod (2k+1)$. Since $2$ is invertible in $\mathbb{Z}_{2 k + 1}$, we can just choose $2^{-1} i$.
-/
